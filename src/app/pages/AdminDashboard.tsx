...
import {
  ...
  Users, Gift, CheckCircle, AlertTriangle, TrendingUp, Ban, Target, MapPin, Calendar, Building
} from 'lucide-react';
...
export function AdminDashboard() {
  ...
  const [selectedOpportunityDetails, setSelectedOpportunityDetails] = useState<VolunteerOpportunity | null>(null);
  ...
  // ── Volunteer applications (admin) state ──────────────────────
  ...
  const fetchVolunteerApplications = async () => {
    ...
    try {
      const res = await api.get('/volunteer/admin/applications');
      setVolunteerApplications(Array.isArray(res.data.data) ? res.data.data : []);
    } catch {
  ...
  useEffect(() => { fetchVerifications(); fetchAdminRequests(); fetchVolunteerApplications(); }, []);
  ...
  const openAddOrg = () => {
    ...
    setOrgModal({ open: true, editing: null });
  };
  ...
  const saveOrg = async () => {
    if (!orgForm.name.trim()) { toast.error('يرجى إدخال اسم المؤسسة'); return; }
    if (!orgForm.description.trim()) { toast.error('يرجى إدخال وصف المؤسسة'); return; }
    if (!orgForm.location.trim()) { toast.error('يرجى إدخال الموقع'); return; }
    
    try {
      if (orgModal.editing) {
        await updateOrganization(orgModal.editing.id, orgForm);
      ...
      const details: any[] = Array.isArray(err.response?.data?.details) ? err.response.data.details : [];

      if (!err.response) {
        // Network / timeout
        toast.error('تعذّر الاتصال بالخادم. تحقق من اتصالك بالإنترنت وأعد المحاولة.');
      } else if (status === 409) {
        toast.error('يوجد مؤسسة بنفس الاسم مسبقاً. يرجى اختيار اسم مختلف.');
      } else if (status === 422 || details.length > 0) {
        const fieldErrors = details.map((d: any) => `• ${d.message}`).join('\n');
        toast.error(
          fieldErrors
            ? `بيانات غير صحيحة:\n${fieldErrors}`
            : serverError || 'تحقق من البيانات المدخلة وأعد المحاولة.',
          { duration: 5000 }
        );
      } else if (status === 403) {
        toast.error('ليس لديك صلاحية لتنفيذ هذا الإجراء.');
      } else if (status === 404) {
        toast.error('المؤسسة غير موجودة. قد تكون حُذفت مسبقاً.');
      } else if (status >= 500) {
        toast.error('حدث خطأ في الخادم. يرجى المحاولة لاحقاً أو التواصل مع الدعم الفني.');
      } else {
        toast.error(serverError || 'حدث خطأ غير متوقع أثناء حفظ المؤسسة. أعد المحاولة.');
      }
    }
  };
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6 max-w-7xl mx-auto">
        ...
        <Tabs value={activeTab} onValueChange={setTab}>
          <TabsList className="flex flex-wrap gap-1 h-auto rounded-xl p-1.5 bg-muted overflow-x-auto">
            ...
            <TabsTrigger value="organizations" className="rounded-lg text-sm font-semibold">المؤسسات</TabsTrigger>
            ...
            <TabsTrigger value="volunteer-applications" className="rounded-lg text-sm font-semibold">طلبات التطوع</TabsTrigger>
                  ...
                  {adminDonationsList.map((d) => (
                    <div key={d.id} className="flex flex-col sm:flex-row sm:items-center gap-3 p-4 rounded-xl border bg-background hover:bg-muted/40 transition-colors">
                      ...
                      <div className="flex gap-2 flex-shrink-0">
                        <Button variant="outline" size="sm" className="h-8 gap-1" onClick={() => setSelectedDonation(d)}>
                  ...
                  {campaigns.map((c) => {
                    ...
                    return (
                      <div key={c.id} className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 rounded-xl border bg-background hover:bg-muted/40 transition-colors">
                        ...
                        <div className="flex gap-2 flex-shrink-0">
                          <Button variant="outline" size="sm" className="h-8 gap-1" onClick={() => setSelectedCampaign(c)}>
                            <Eye className="h-3.5 w-3.5" /> عرض
                          </Button>
                  ...
                  {opportunities.map((opp) => {
                    ...
                    return (
                      <div key={opp.id} className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 rounded-xl border bg-background hover:bg-muted/40 transition-colors">
                        ...
                        <div className="flex gap-2 flex-shrink-0">
                          <Button variant="outline" size="sm" className="h-8 gap-1" onClick={() => setSelectedOpportunityDetails(opp)}>
                            <Eye className="h-3.5 w-3.5" /> عرض
                          </Button>
          ...
          <TabsContent value="volunteer-applications" className="mt-6">
            <Card className="border-none card-shadow rounded-3xl bg-card overflow-hidden">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle>طلبات التطوع</CardTitle>
                    <CardDescription>عرض طلبات المتطوعين للموافقة أو الرفض</CardDescription>
                  </div>
                  <Button variant="outline" size="sm" onClick={fetchVolunteerApplications}>تحديث</Button>
                </div>
              </CardHeader>
              <CardContent>
                {volunteerApplicationsLoading ? (
                  ...
                  <div className="text-center py-10 text-muted-foreground">لا توجد طلبات تطوع حالياً</div>
                ) : (
                  <div className="space-y-4">
                    {volunteerApplications.map((opp) => (
                      <Card key={opp.id} className="border-none card-shadow rounded-2xl">
                        <CardContent className="p-4 space-y-3">
                          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                            <div>
                              <p className="font-semibold">{opp.title}</p>
                              <p className="text-sm text-muted-foreground">{opp.description}</p>
                              <p className="text-xs text-muted-foreground mt-1">الموقع: {opp.location} • المتقدمون: {opp.applicants?.length || 0}</p>
                            </div>
                            <Badge className="bg-secondary/10 text-secondary text-xs">{opp.isActive ? 'نشط' : 'غير نشط'}</Badge>
                          </div>

                          {opp.applicants?.length > 0 ? (
                            <div className="space-y-3">
                              {opp.applicants.map((applicant: any) => (
                                <div key={applicant._id} className="flex flex-col sm:flex-row sm:items-center gap-3 p-3 rounded-xl border bg-background">
                                  <div className="flex-1 min-w-0">
                                    <p className="font-medium">{applicant.name}</p>
                                    <p className="text-xs text-muted-foreground" dir="ltr">{applicant.email}</p>
                                    <p className="text-xs text-muted-foreground">الدور: {applicant.role === 'volunteer' ? 'متطوع' : 'مستخدم'}</p>
                                  </div>
                                  <div className="flex gap-2 flex-shrink-0">
                                    <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white gap-1" onClick={async () => {
                                      try {
                                        await api.put(`/volunteer/${opp.id}/applicants/${applicant._id}/approve`);
                                    ...
                                    <Button size="sm" variant="outline" className="text-destructive border-destructive/40 gap-1" onClick={async () => {
                                      try {
                                        await api.put(`/volunteer/${opp.id}/applicants/${applicant._id}/reject`);
                            ...
                            <div className="text-xs text-muted-foreground">لا يوجد متقدمون على هذه الفرصة بعد.</div>
                          )}
                        </CardContent>
                      </Card>
                    ))}
                  ...
                  {donations.slice(0, 5).map((d) => (
                    <div key={d.id} className="flex flex-col sm:flex-row sm:items-center gap-3 p-4 rounded-xl border bg-background hover:bg-muted/40 transition-colors">
                      <div className="flex-1 min-w-0">
                        ...
                        <Button variant="outline" size="sm" className="h-8 gap-1 flex-shrink-0" onClick={() => setSelectedDonation(d)}>
                          <Eye className="h-3.5 w-3.5" /> عرض
                        </Button>
                    </div>
                  ))}
            ...
            <Card className="border-none card-shadow rounded-3xl bg-card overflow-hidden">
              <CardHeader className="flex flex-row items-start justify-between">
                <div>
                  ...
                  <CardDescription>{organizations.length} مؤسسة مسجلة</CardDescription>
                </div>
                <Button onClick={openAddOrg} className="gap-2">
              ...
              <CardContent>
                {organizations.length === 0 ? (
                  <div className="text-center py-12">
                    <Building className="h-12 w-12 text-muted-foreground/40 mx-auto mb-4" />
                    <p className="text-muted-foreground">لا توجد مؤسسات حالياً</p>
                    <Button variant="outline" className="mt-4" onClick={openAddOrg}>أضف أول مؤسسة</Button>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {organizations.map((org) => (
                      <div key={org.id} className="flex flex-col sm:flex-row sm:items-center gap-3 p-4 rounded-xl border bg-background hover:bg-muted/40 transition-colors">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          {org.image ? (
                            <img src={org.image} alt={org.name} className="w-12 h-12 rounded-xl object-cover flex-shrink-0" />
                          ) : (
                          ...
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <Badge variant="outline" className="text-xs gap-1"><MapPin className="h-3 w-3" />{org.location}</Badge>
                            </div>
                        ...
                        <div className="flex items-center gap-4 text-xs text-muted-foreground flex-shrink-0">
                          <div className="text-center">
                            <p className="font-bold text-foreground text-sm">{org.raisedFunds?.toLocaleString() || 0}</p>
                            <p>دينار</p>
                          </div>
                          <div className="text-center">
                            <p className="font-bold text-foreground text-sm">{org.volunteersCount || 0}</p>
                            <p>متطوع</p>
                          </div>
                        </div>
                        <div className="flex gap-2 flex-shrink-0">
                          <Button variant="outline" size="sm" className="h-8 gap-1" onClick={() => openEditOrg(org)}>
                            <Pencil className="h-3.5 w-3.5" /> تعديل
                          </Button>
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => setDeleteOrgId(org.id)}>
                            <Trash2 className="h-4 w-4" />
                          </Button>
                        </div>
                      </div>
      ...
      <Dialog open={!!selectedDonation} onOpenChange={(open) => { if (!open) setSelectedDonation(null); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>تفاصيل التبرع</DialogTitle>
          </DialogHeader>
          {selectedDonation && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-semibold">{selectedDonation.title}</p>
                </div>
                <div>
                ...
                <div>
                  <p className="text-xs text-muted-foreground">الحالة</p>
                  <p className="font-semibold">{selectedDonation.status}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">مستوى الأهمية</p>
                  <p className="font-semibold">{selectedDonation.urgency || 'عادي'}</p>
                </div>
              </div>
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground">الوصف</p>
                <p>{selectedDonation.description || 'لا يوجد وصف.'}</p>
              </div>
              <div className="grid grid-cols-2 gap-4 text-xs text-muted-foreground">
                <div>الموقع: {selectedDonation.location || 'غير محدد'}</div>
                <div>المتبرع: {selectedDonation.donor?.name || 'غير معروف'}</div>
                <div>تاريخ الإنشاء: {new Date(selectedDonation.createdAt).toLocaleDateString('ar-SA')}</div>
              </div>
            </div>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setSelectedDonation(null)}>إغلاق</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          {selectedCampaign && (
            <div className="space-y-4">
              {selectedCampaign.image && (
                <img src={selectedCampaign.image} alt={selectedCampaign.title} className="w-full rounded-2xl object-cover max-h-56" />
              )}
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground">العنوان</p>
              ...
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground">الوصف</p>
                <p>{selectedCampaign.description || 'لا يوجد وصف.'}</p>
              </div>
              <div className="grid grid-cols-2 gap-4 text-xs text-muted-foreground">
                <div>الهدف: {selectedCampaign.target}</div>
                <div>تم جمعه: {selectedCampaign.current}</div>
                <div>الأولوية: {selectedCampaign.urgency}</div>
                <div>الحالة: {selectedCampaign.isActive ? 'نشطة' : 'متوقفة'}</div>
              </div>
            </div>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setSelectedCampaign(null)}>إغلاق</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* ── Volunteer Opportunity Details Modal ─────────────────── */}
      <Dialog open={!!selectedOpportunityDetails} onOpenChange={(open) => { if (!open) setSelectedOpportunityDetails(null); }}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>تفاصيل فرصة التطوع</DialogTitle>
          </DialogHeader>
          {selectedOpportunityDetails && (
            <div className="space-y-4">
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground">العنوان</p>
                <p className="font-semibold text-lg">{selectedOpportunityDetails.title}</p>
              </div>
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground">الوصف</p>
                <p>{selectedOpportunityDetails.description || 'لا يوجد وصف.'}</p>
              </div>
              <div className="grid grid-cols-2 gap-4 text-xs text-muted-foreground">
                <div>الموقع: {selectedOpportunityDetails.location}</div>
                <div>التاريخ: {selectedOpportunityDetails.date ? new Date(selectedOpportunityDetails.date).toLocaleDateString('ar-SA') : 'غير محدد'}</div>
                <div>المتطوعون: {selectedOpportunityDetails.volunteers}/{selectedOpportunityDetails.maxVolunteers}</div>
                <div>الحالة: {selectedOpportunityDetails.isActive ? 'نشطة' : 'مغلقة'}</div>
              </div>
            </div>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setSelectedOpportunityDetails(null)}>إغلاق</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={orgModal.open} onOpenChange={(open) => { if (!open) setOrgModal({ open: false, editing: null }); }}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{orgModal.editing ? 'تعديل المؤسسة' : 'إضافة مؤسسة جديدة'}</DialogTitle>
            <DialogDescription>{orgModal.editing ? 'قم بتعديل بيانات المؤسسة' : 'أدخل بيانات المؤسسة الجديدة'}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1 sm:col-span-2">
                <label className="text-sm font-medium">اسم المؤسسة <span className="text-destructive">*</span></label>
                <Input placeholder="مثال: مؤسسة الخير" value={orgForm.name} onChange={(e) => setOrgForm((p) => ({ ...p, name: e.target.value }))} />
              </div>
              <div className="space-y-1 sm:col-span-2">
                <label className="text-sm font-medium">الوصف <span className="text-destructive">*</span></label>
                <Textarea placeholder="وصف المؤسسة وأهدافها..." rows={3} value={orgForm.description} onChange={(e) => setOrgForm((p) => ({ ...p, description: e.target.value }))} />
              </div>
              <div className="space-y-1 sm:col-span-2">
                <label className="text-sm font-medium">رابط الصورة</label>
                <Input placeholder="https://example.com/image.jpg" dir="ltr" value={orgForm.image} onChange={(e) => setOrgForm((p) => ({ ...p, image: e.target.value }))} />
              </div>
              <div className="space-y-1">
                <label className="text-sm font-medium">الموقع <span className="text-destructive">*</span></label>
                <Select value={orgForm.location} onValueChange={(val) => setOrgForm((p) => ({ ...p, location: val }))}>
                  <SelectTrigger>
                    <SelectValue placeholder="اختر الموقع" />
                  </SelectTrigger>
                  <SelectContent>
                    {['عمان', 'إربد', 'الزرقاء', 'العقبة', 'جرش', 'عجلون', 'الكرك', 'مادبا', 'السلط', 'المفرق'].map((loc) => (
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium">البريد الإلكتروني</label>
                <Input placeholder="info@example.org" dir="ltr" value={orgForm.contactEmail} onChange={(e) => setOrgForm((p) => ({ ...p, contactEmail: e.target.value }))} />
              </div>
              <div className="space-y-1 sm:col-span-2">
                <label className="text-sm font-medium">رقم الهاتف</label>
                <Input placeholder="+962 7X XXX XXXX" dir="ltr" value={orgForm.contactPhone} onChange={(e) => setOrgForm((p) => ({ ...p, contactPhone: e.target.value }))} />
              </div>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setOrgModal({ open: false, editing: null })}>إلغاء</Button>
            <Button onClick={saveOrg}>{orgModal.editing ? 'حفظ التعديلات' : 'إضافة المؤسسة'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={!!deleteOrgId} onOpenChange={() => setDeleteOrgId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>هل أنت متأكد من حذف هذه المؤسسة؟ سيتم حذف جميع بياناتها بشكل نهائي ولا يمكن التراجع عن هذا الإجراء.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>إلغاء</AlertDialogCancel>
            <AlertDialogAction className="bg-destructive text-white hover:bg-destructive/90" onClick={handleDeleteOrg}>حذف المؤسسة</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
