...
export type UserType = 'beneficiary';
export type AccountType = 'beneficiary' | 'volunteer' | 'donor';
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const loginWithGoogle = async () => {
    if (!firebaseEnabled) {
      return { success: false, error: 'تسجيل الدخول بجوجل غير متاح حالياً. الرجاء استخدام البريد الإلكتروني.' };
    }
  ...
  const signup = async (name: string, email: string, password: string, accountType: AccountType = 'donor', phone?: string) => {
    try {
      const payload: Record<string, any> = { name, email, password, phone };
      if (accountType === 'volunteer') {
        payload.role = 'volunteer';
      } else if (accountType === 'beneficiary') {
        payload.user_type = 'beneficiary';
      } // donor: no extra fields
