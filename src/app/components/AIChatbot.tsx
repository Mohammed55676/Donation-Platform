...
export function AIChatbot() {
  ...
  const getLocalFallbackAnswer = (text: string, isEnglish: boolean): string => {
    ...
    if (isEnglish) {
      if (
        query.match(/^(hi|hello|hey|howdy|greetings|good morning|good afternoon|good evening|what'?s up|sup)[^a-z]?$/) ||
        query === 'hi' || query === 'hello' || query === 'hey' ||
        query.includes('how are you') || query.includes('how r u') || query.includes('how do you do')
      ) {
        return "Hello! 👋 Welcome to Al-Khair Platform. I'm here to help you with donations, volunteering, or anything else on the platform. What can I do for you?";
      }
      if (query.includes('thank') || query.includes('thanks') || query.includes('thx') || query.includes('ty')) {
        return "You're welcome! 😊 If you need anything else, feel free to ask.";
      }
      if (query.includes('bye') || query.includes('goodbye') || query.includes('see you') || query.includes('take care')) {
        return 'Goodbye! Have a great day. Feel free to come back anytime. 😊';
      }
      if (query.includes('who are you') || query.includes('what are you') || query.includes('what can you do')) {
        return "I'm the Al-Khair Platform smart assistant. I can help you with donations, volunteering, requesting help, account issues, and more. Just ask!";
      }
      ...
      if (
        query.includes('مرحبا') || query.includes('مرحبً') || query.includes('أهلا') || query.includes('اهلا') ||
        query.includes('هلا') || query.includes('هلو') || query.includes('السلام عليكم') || query.includes('السلام') ||
        query.includes('يا هلا') || query.includes('حياك') || query.includes('أهلين') || query.includes('مرحبتين') ||
        query.includes('صباح الخير') || query.includes('مساء الخير') || query.includes('صباح النور') || query.includes('مساء النور') ||
        query === 'هي' || query === 'هاي' || query === 'هلو'
      ) {
        return 'أهلاً وسهلاً! 👋 مرحباً بك في منصة الخير. أنا هنا لمساعدتك في كل ما يتعلق بالتبرعات والتطوع وطلبات المساعدة. كيف يمكنني خدمتك؟';
      }
      if (query.includes('كيف حالك') || query.includes('كيف الحال') || query.includes('كيف أحوالك') || query.includes('شو أخبارك') || query.includes('ايش أخبارك') || query.includes('إيش أخبارك')) {
        return 'بخير والحمد لله! 😊 شكراً لسؤالك. أنا دائماً هنا ومستعد لمساعدتك. ماذا تحتاج؟';
      }
      if (query.includes('شكرا') || query.includes('شكراً') || query.includes('مشكور') || query.includes('يسلمو') || query.includes('تسلم')) {
        return 'العفو! 😊 سعيد بمساعدتك. إذا احتجت أي شيء آخر لا تتردد في السؤال.';
      }
      if (query.includes('مع السلامة') || query.includes('وداعاً') || query.includes('باي') || query.includes('الله يحفظك') || query.includes('يعطيك العافية')) {
        return 'مع السلامة! 😊 يسعدنا دائماً مساعدتك. إلى اللقاء!';
      }
      if (query.includes('من أنت') || query.includes('من انت') || query.includes('ما هو دورك') || query.includes('ماذا تفعل') || query.includes('وش تسوي')) {
        return 'أنا المساعد الذكي لمنصة الخير! 🤖 يمكنني مساعدتك في التبرعات، التطوع، طلبات المساعدة، مشاكل الحساب، وأي استفسار عن المنصة. فقط اسألني!';
      }
              ...
              <div className="px-4 py-2 bg-muted/10 border-t border-border/20 flex flex-wrap gap-1.5 justify-center">
                {quickActions.map((action, idx) => (
                  <button
                    ...
                    className="text-[11px] font-medium px-3 py-1.5 rounded-full bg-background border border-border/50 hover:border-primary hover:text-primary transition-all duration-200 cursor-pointer shadow-xs active:scale-95"
                  >
                    {action}
                  </button>
                ))}
