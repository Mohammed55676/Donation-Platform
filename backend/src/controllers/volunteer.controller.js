...
async function adminListApplications(req, res, next) {
  try {
    const opportunities = await VolunteerOpportunity.find()
      ...
      .populate('applicants', 'name email avatar role')
      .sort({ createdAt: -1 });

    return sendSuccess(res, opportunities, 'Volunteer applications retrieved.', 200);
  } catch (err) {
    next(err);
  }
}

async function adminRejectApplicant(req, res, next) {
  try {
    const { id, userId } = req.params;
    const opp = await VolunteerOpportunity.findById(id);
    if (!opp) throw new AppError('Opportunity not found.', 404);

    const index = opp.applicants.findIndex((applicant) => applicant.equals(userId));
    if (index === -1) throw new AppError('Applicant not found for this opportunity.', 404);

    opp.applicants.splice(index, 1);
    opp.volunteers = Math.max(0, opp.volunteers - 1);
    await opp.save();

    return sendSuccess(res, opp, 'Applicant rejected.', 200);
  } catch (err) {
    next(err);
  }
}

async function adminApproveApplicant(req, res, next) {
  try {
    const { id, userId } = req.params;
    const opp = await VolunteerOpportunity.findById(id).populate('applicants', 'name email avatar role');
    if (!opp) throw new AppError('Opportunity not found.', 404);

    const exists = opp.applicants.some((applicant) => applicant._id.equals(userId));
    if (!exists) throw new AppError('Applicant not found for this opportunity.', 404);

    return sendSuccess(res, opp, 'Applicant approved.', 200);
  } catch (err) {
    next(err);
  }
}
